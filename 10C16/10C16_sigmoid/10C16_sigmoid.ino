...
void loop() {
  ...
  {
    gateOpen = true;
    SigmoidMove(startAngle, stopAngle, 2500);
    delay(2000);
  }

  if (dist_median > 80 && gateOpen == true)
  {
    SigmoidMove(stopAngle, startAngle, 2500);
    gateOpen = false;
  }
...
void SigmoidMove(int start_Angle, int stop_Angle, int duration) {
  int steps = 100;
  float delayTime = (float)duration / steps;
  float angleChange = stop_Angle - start_Angle;

  for (int i = 0; i <= steps; i++) {
    float input = -6.0 + ((float)i / steps) * 12.0;

    float sigmoidOutput = 1.0 / (1.0 + exp(-input));

    int currentAngle = start_Angle + (angleChange * sigmoidOutput);
